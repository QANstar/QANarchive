...
const ASSET_ACCEPT = '.fbx,.blend,.zip,.unitypackage';
    ...
    <div className="asset-uploader">
      <div className="asset-uploader-row">
        <button type="button" className="btn btn-plain btn-sm" onClick={() => assetRef.current?.click()}>
          {asset ? `已选: ${asset.name}` : '选择 3D 文件 (.fbx/.blend/.zip/.unitypackage)'}
        </button>
      </div>
      <div className="asset-uploader-actions">
        ...
        <span className="hint">fbx ≤200MB · blend/zip/unitypackage ≤900MB · 封面使用作品自身的媒体图片</span>
      </div>
